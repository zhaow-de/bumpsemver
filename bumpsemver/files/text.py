...
class ConfiguredPlainTextFile(FileTypeBase):
    ...
    def __init__(self, path, version_config: VersionConfig):
        self.path = path
    ...
    def should_contain_version(self, version: Version, context: dict) -> None:
        ...
        if search_pattern_is_default and self.contains(version.original):
            # original version is present, and we're not looking for something more specific
        ...
        with open(self.path, "rt", encoding="utf-8") as orig_fp:
            ...
            for lineno, line in enumerate(orig_fp.readlines()):
                ...
                if (
                    ...
                    logger.info(
                        f"Found '{search}' in {self.path} at line {lineno - (len(lookbehind) - 1)}: {line.rstrip()}"
                    )
                    return True
        return False
    ...
    def __repr__(self):
        return f"<bumpsemver.files.ConfiguredPlainTextFile:{self.path}>"
